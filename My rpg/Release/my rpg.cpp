...
void faq() {
	...
	cout << endl;
	cout << "Unfortunately, you will need to download some runtime requirements in order for the game to function. I'm actively searching for a way around this.\n\n" << endl;
...
void err() {
	...
	if (lvl > 3) {
		error << "lvl is more than 3, "; 
	}
	if (herofullh > 15) {
		error << "herofullh is more than 15, ";
	}
	if (herostrength > 14) {
		error << "herostrength is more than 14, ";
	}
	if (herodef > 6) {
		error << "herodef is more than 6, ";
	}
	if (spfull > 11) {
		error << "spfull is more than 11, ";
	}
	if (state > 3) {
	...
	if (herodef <= 0) {
		error << "herodef is less or equal to 0, ";
	}
...
checkagain:
	...
	if (xp >= leveluprequirement && preplannedlevelup == false) {
				recheck = true;
				lvl++;
				levelup++;
				statraise = true;
...
void cheatcommandmenu() {
	int herofullhnew = herofullh;
	int herodefnew = herodef;
	int statenew = state;
	int spfullnew = spfull;
	int moneynew = money;
	int herostrengthnew = herostrength;
	int lvlnew = lvl;
	...
	while (cheatcommand != "9") {
		system("cls");
		if (herofullh != herofullhnew || herostrength != herostrengthnew || lvl != lvlnew || herodef != herodefnew || money != moneynew || state != statenew || spfull != spfullnew || cheatinvincibility != cheatinvincibilitynew || monsterdoesnothing != monsterdoesnothingnew || twoturnsinrow != twoturnsinrownew) {
...
if (cheatcommand == "10") {
		if (herofullh != herofullhnew || herostrength != herostrengthnew || lvl != lvlnew || herodef != herodefnew || money != moneynew || state != statenew || spfull != spfullnew || cheatinvincibility != cheatinvincibilitynew || monsterdoesnothing != monsterdoesnothingnew || twoturnsinrow != twoturnsinrownew) {
			...
			if (cheatcommand == "y" || cheatcommand == "Y") {
				herofullh = herofullhnew;
				herostrength = herostrengthnew;
				lvl = lvlnew;
				herodef = herodefnew;
				money = moneynew;
				state = statenew;
...
void integritycheck()  {
	...
	cout << "Verifying integrity of file..." << endl;
		if (lvl > 5 || herofullh > 25 || herostrength > 24 || herodef > 10 || herodef <=0 || spfull > 21 || state > 3 || money > 1000 || weapon > 1 || integritychkskip > 1 || item1has > 1 || item2has > 1 || item3has > 0 || item4has > 0 || item5has > 0 || item1type > 2 || item2type > 2 || item3type > 2 || item4type > 2 || item5type > 2) {
		if (auth == true) {
...
void gamesavehandler() {
	...
	if (save == Auto || save == Save_available) {
		...
		switch (itemmenu)  {
		case 1:
			writesave << integritychkskip << endl << leveluprequirement << endl << oldrequirement << endl << lvl << endl << spfull << endl << herodef << endl << herostrength << endl << herofullh << endl << xp << endl << levelup << endl << heroname << endl << state << endl << money << endl << weapon << endl << item1 << endl << item1a << endl << item1has << endl << item1type << endl;
			break;
		case 2:
			writesave << integritychkskip << endl << leveluprequirement << endl << oldrequirement << endl << lvl << endl << spfull << endl << herodef << endl << herostrength << endl << herofullh << endl << xp << endl << levelup << endl << heroname << endl << state << endl << money << endl << weapon << endl << item1 << endl << item1a << endl << item1has << endl << item1type << endl << item2 << endl << item2a << endl << item2has << endl << item2type << endl;
			break;
		case 3:
		...
		default:
			...
			writesave << integritychkskip << endl << leveluprequirement << endl << oldrequirement << endl << lvl << endl << spfull << endl << herodef << endl << herostrength << endl << herofullh << endl << xp << endl << levelup << endl << heroname << endl << state << endl << money << endl << weapon << endl;
			break;
		}
		writesave.close();
		system("cls");
	...
	if (readsave.fail()) {
		while (true) {
			...
			if (alert == IDYES) {
				...
				ofstream writesave;
				writesave.open("gamesave.dat");
				writesave << integritychkskip << endl << leveluprequirement << endl << oldrequirement << endl << lvl << endl << spfull << endl << herodef << endl << herostrength << endl << herofullh << endl << xp << endl << levelup << endl << heroname << endl << state << endl << money << endl;
				if (sound == true) {
					Beep(800, 220);
				}
	...
	readsave >> integritychkskip >> leveluprequirement >> oldrequirement >> lvl >> spfull >> herodef >> herostrength >> herofullh >> xp >> levelup >> heroname >> state >> money >> weapon >> item1 >> item1read >> item1a >> item1has >> item1type >> item2 >> item2read >> item2a >> item2has >> item2type >> item3 >> item3read >> item3a >> item3has >> item3type >> item4 >> item4read >> item4a >> item4has >> item4type >> item5 >> item5read >> item5a >> item5has >> item5type;
...
int main() {
	...
	int alert; 
	MessageBox(NULL, "This will be the last iteration of COMPUTERDOOM betas on version 1.0. Version 1.0 will not be happening due to an exciting change. See \"FAQs\" in the main menu for more details.",
	...
	money = money + 100;
		...
		system("pause");
		cout << "\nThis is going to be difficult, so take 200 coins. I've served my purpose, you're on your own " << heroname << " .Good luck!\n" << endl;
		system("pause");
		alert = MessageBox(NULL, "This is going to be intense! Would you like to go to the store to spend your coins?", "You're gonna need more firepower", MB_YESNO | MB_ICONQUESTION);
